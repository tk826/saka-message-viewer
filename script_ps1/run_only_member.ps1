...
if ($logAvailable) {
    & '.\venv_win\Scripts\python.exe' @pyArgs 2>&1 | ForEach-Object { "$_" } | Tee-Object -FilePath $logPath -Append
    Write-Output "EXITCODE:$LASTEXITCODE" | Tee-Object -FilePath $logPath -Append
} else {
    & '.\venv_win\Scripts\python.exe' @pyArgs 2>&1 | ForEach-Object { "$_" }
